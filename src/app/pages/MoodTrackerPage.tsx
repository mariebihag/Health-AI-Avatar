...
const JOURNAL_ENTRIES = [
  { mood:'happy',     note:'Had a great workout and met friends for lunch. Feeling productive.', time:'Today, 2:30 PM', words:12 },
  { mood:'calm',      note:'Meditated for 20 minutes in the morning. Really helped my focus.', time:'Yesterday, 8:00 AM', words:11 },
  { mood:'energetic', note:'Crushed my step goal and felt amazing all day. Best day this week!', time:'Wed, 6:15 PM', words:13 },
  { mood:'tired',     note:'Long day at work. Skipped dinner and went to bed early.', time:'Thu, 9:00 PM', words:10 },
];
...
export function MoodTrackerPage() {
  ...
  const moodCounts = MOODS.map(m => ({
    ...m,
    count: JOURNAL_ENTRIES.filter(e => e.mood === m.id).length,
    ...
    <>
      <style>{`
        ...
        @keyframes glowPulse {
          ...
          50%    { box-shadow: 0 0 30px 4px var(--glow); }
        }
        /* Slide in from right */
        ...
        .submit-btn:hover { transform:translateY(-2px) !important; box-shadow:0 10px 30px var(--btn-glow) !important; }
                ...
                {step === 'done' ? (
                  ...
                  <div style={{ width:'100%', animation:'slideRight 0.3s ease' }}>
                    ...
                    <div style={{ display:'flex', gap:'10px', marginTop:'14px' }}>
                      ...
                      <button
                        ...
                        style={{ flex:1, padding:'11px', background:`linear-gradient(135deg,${displayMood?.color},${displayMood?.color}99)`, border:'none', borderRadius:'12px', color:'#fff', fontWeight:700, fontSize:'14px', cursor:'pointer', boxShadow:`0 4px 18px ${displayMood?.glow}50`, transition:'all .2s', ['--btn-glow' as any]: displayMood?.glow }}>
                        Save Mood
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                    {activeMood ? (
                      <>
                        <img
                          key={`big-${hoveredMood || selectedMood}`}
                          src={activeMood.img}
                  ...
                  {MOODS.map((mood) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          border: isSelected ? `1.5px solid ${mood.color}88` : '1.5px solid rgba(255,255,255,0.08)',
                          background: isSelected ? mood.bg : 'rgba(255,255,255,0.04)',
                          boxShadow: isSelected ? `0 0 20px ${mood.glow}40, 0 4px 16px rgba(0,0,0,0.3)` : '0 2px 8px rgba(0,0,0,0.2)',
                      ...
                      >
                        <img
                          src={mood.img}
                          alt={mood.label}
                          style={{ width:52, height:52, objectFit:'contain', animation: isSelected ? `moodPop 0.4s ease` : 'none', filter: isSelected ? `drop-shadow(0 0 10px ${mood.glow})` : 'none', transition:'filter 0.2s' }}
                        />
                    ...
                    return (
                      <div key={d.day} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'6px', animation: mounted ? `fadeUp 0.4s ease ${0.05*i}s both` : 'none' }}>
                        ...
                        <div style={{
                          ...
                          background: m ? m.bg : 'rgba(255,255,255,0.04)',
                          border: isToday ? `2px solid ${m?.color || '#a78bfa'}` : `1px solid ${m ? m.color+'44' : 'rgba(255,255,255,0.08)'}`,
                          display:'flex', alignItems:'center', justifyContent:'center',
                          boxShadow: m ? `0 0 12px ${m.glow}30` : 'none',
                          transition:'all .3s',
                        }}>
                          {m ? (
                ...
                <div style={{ display:'flex', gap:'4px', marginBottom:'20px', background:'rgba(255,255,255,0.04)', borderRadius:'12px', padding:'4px' }}>
                  {(['today','history','insights'] as const).map(tab => (
                    <button key={tab} className="tab-btn"
                      style={{ flex:1, background: activeTab === tab ? 'rgba(167,139,250,0.2)' : 'transparent', color: activeTab === tab ? '#a78bfa' : 'rgba(180,210,255,0.45)', borderColor: activeTab === tab ? 'rgba(167,139,250,0.3)' : 'transparent', border: activeTab === tab ? '1px solid rgba(167,139,250,0.3)' : '1px solid transparent' }}
                ...
                {activeTab === 'today' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:'12px', animation:'fadeIn 0.3s ease' }}>
                    <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:'12px' }}>
                      {[
                        { label:'Current Mood', value: displayMood?.label || '—', color: displayMood?.color || '#94a3b8', icon:'😊' },
                        { label:'Mood Streak',  value:'4 days', color:'#fbbf24', icon:'🔥' },
                        { label:'Best Mood',    value:'Happy',  color:'#fbbf24', icon:'⭐' },
                        { label:'Entries',      value:`${JOURNAL_ENTRIES.length} this week`, color:'#34d399', icon:'📝' },
                      ].map(s => (
                    ...
                    <div style={{ background:'rgba(167,139,250,0.08)', border:'1px solid rgba(167,139,250,0.2)', borderRadius:'14px', padding:'16px' }}>
                      ...
                      <p style={{ color:'#e0f0ff', fontSize:'13px', margin:0, lineHeight:1.6 }}>
                        Your mood has been mostly <span style={{ color:'#fbbf24', fontWeight:700 }}>positive</span> this week. Happy and Energetic moods dominate. Keep maintaining your healthy habits!
                      </p>
                    </div>
                  </div>
                )}
                      ...
                      return (
                        <div key={i} className="journal-item" style={{ display:'flex', gap:'12px', padding:'14px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(100,180,255,0.08)', borderRadius:'14px', transition:'all .2s', cursor:'default' }}>
                          ...
                          <div style={{ flex:1, minWidth:0 }}>
                            ...
                            <p style={{ color:'rgba(180,210,255,0.65)', fontSize:'12px', margin:0, lineHeight:1.5, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{entry.note}</p>
                ...
                {activeTab === 'insights' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:'14px', animation:'fadeIn 0.3s ease' }}>
                    <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr', gap:'10px' }}>
                      {[
                        { label:'Total Entries', value:'172', color:'#a78bfa' },
                        { label:'Total Words',   value:'27.3K', color:'#38bdf8' },
                      ...
                      <div key={m.id} style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <div style={{ height:'4px', borderRadius:'2px', background:'rgba(255,255,255,0.06)' }}>
                            <div style={{ height:'100%', borderRadius:'2px', background:m.color, width:`${(m.count/4)*100}%`, boxShadow:`0 0 6px ${m.glow}`, transition:'width 0.8s ease' }} />
                          </div>
                        </div>
                        {i === 0 && (
