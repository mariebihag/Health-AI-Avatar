...
function useStepCounter() {
  ...
  return { steps, isTracking, error, supported, startTracking, stopTracking, resetSteps };
}
...
export function StepsPage() {
  ...
  const loadTodaySteps = async () => {
    try {
      ...
      const todayDocs = res.documents.filter(doc => doc.userID === user.$id && doc.date === today);
  ...
  const handleSaveSensorSteps = async () => {
    ...
    try {
      ...
      setCurrentSteps(prev => prev + sensorSteps);
      resetSteps();
      setShowSaveModal(false);
      toast.success(`${sensorSteps} sensor steps saved!`);
    } catch (err) {
      console.error('❌ Save sensor steps error:', err);
      toast.error('Failed to save sensor steps.');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="main-content" style={{ padding: 0 }}>
          ...
          <div style={{ padding: isMobile ? '16px' : '24px 28px', display: 'grid', gridTemplateColumns: isMobile || isTablet ? '1fr' : '1fr 320px', gap: '22px', minHeight: 'calc(100vh - 73px)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{
                ...card,
                border: isTracking ? '1px solid rgba(34,197,94,0.5)' : '1px solid rgba(34,197,94,0.2)',
                boxShadow: isTracking ? '0 0 30px rgba(34,197,94,0.15)' : 'none',
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px', flexWrap: 'wrap', gap: '10px' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                      {isTracking && (
                        <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e', animation: 'sensorPulse 1.5s ease-in-out infinite' }} />
                      )}
                      <p style={{ color: 'rgba(180,210,255,0.45)', fontSize: '11px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' as const, margin: 0 }}>
                        {isTracking ? 'Live Tracking' : 'Auto Detect'}
                      </p>
                    </div>
                    <p style={{ color: '#e0f0ff', fontWeight: 700, fontSize: '15px', margin: 0 }}>📱 Accelerometer Step Counter</p>
                  </div>
                  {sensorSteps > 0 && !isTracking && (
                    <button
                      onClick={() => setShowSaveModal(true)}
                      style={{ padding: '8px 16px', background: 'linear-gradient(135deg,#22c55e,#16a34a)', border: 'none', borderRadius: '10px', color: '#fff', fontWeight: 700, fontSize: '12px', cursor: 'pointer', boxShadow: '0 4px 14px rgba(34,197,94,0.35)' }}>
                      💾 Save to Log
                    </button>
                  )}
                </div>
                ...
                <div style={{ textAlign: 'center', marginBottom: '20px', padding: '20px', background: 'rgba(34,197,94,0.05)', borderRadius: '14px', border: '1px solid rgba(34,197,94,0.1)' }}>
                  ...
                  <p style={{ color: 'rgba(180,210,255,0.5)', fontSize: '13px', margin: '6px 0 0', fontWeight: 600 }}>
                    steps detected this session
                  </p>
                  {sensorSteps > 0 && (
                ...
                {sensorSteps > 0 && (
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    <div style={{ height: '8px', borderRadius: '4px', background: 'rgba(255,255,255,0.07)', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${sensorPct}%`, background: 'linear-gradient(90deg,#22c55e,#4ade80)', borderRadius: '4px', boxShadow: '0 0 10px rgba(34,197,94,0.5)', transition: 'width 0.5s ease' }} />
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  {!isTracking ? (
                    <button
                      ...
                      style={{
                        flex: 1, padding: '13px', borderRadius: '12px', border: 'none', cursor: sensorSupported ? 'pointer' : 'not-allowed',
                        background: sensorSupported ? 'linear-gradient(135deg,#22c55e,#16a34a)' : 'rgba(255,255,255,0.05)',
                        color: sensorSupported ? '#fff' : 'rgba(180,210,255,0.3)',
                        fontWeight: 700, fontSize: '14px',
                        boxShadow: sensorSupported ? '0 4px 18px rgba(34,197,94,0.4)' : 'none',
                        transition: 'all 0.2s',
                      }}>
                      🚶 Start Auto Tracking
                    </button>
                    ...
                    <button
                      ...
                      style={{ flex: 1, padding: '13px', borderRadius: '12px', border: '1px solid rgba(239,68,68,0.4)', cursor: 'pointer', background: 'rgba(239,68,68,0.15)', color: '#f87171', fontWeight: 700, fontSize: '14px', transition: 'all 0.2s' }}>
                      ⏹ Stop Tracking
                    </button>
                  )}
                  <button
                    onClick={resetSteps}
                    style={{ padding: '13px 16px', borderRadius: '12px', border: '1px solid rgba(100,180,255,0.15)', cursor: 'pointer', background: 'rgba(255,255,255,0.04)', color: 'rgba(180,210,255,0.6)', fontWeight: 700, fontSize: '13px', transition: 'all 0.2s' }}>
                    ↺ Reset
                  </button>
                </div>
          ...
          <div style={{ background: '#0d1a38', border: '1px solid rgba(34,197,94,0.35)', borderRadius: '22px', padding: '36px', width: '100%', maxWidth: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.7)', animation: 'fadeUp .3s ease' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button onClick={() => setShowSaveModal(false)}
                style={{ flex: 1, padding: '13px', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(100,180,255,0.2)', borderRadius: '12px', color: 'rgba(180,210,255,0.8)', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>
                Cancel
              </button>
            </div>
          </div>
