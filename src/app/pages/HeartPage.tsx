...
// Memoized Sidebar — never re-renders when BPM state changes
...
export function HeartPage() {
  ...
  // bpmKey triggers the CSS pop animation on the BPM number only — Sidebar is unaffected
    ...
    <>
      ...
      <div className="dashboard-page">
        ...
        <div className="main-content" style={{ padding: '0' }}>
          ...
          {/* Header */}
          <div style={{
            background: 'rgba(8,20,50,0.7)', backdropFilter: 'blur(20px)',
            borderBottom: '1px solid rgba(100,180,255,0.12)',
            padding: '18px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            animation: 'fadeIn 0.4s ease',
          }}>
            <div>
              <h1 style={{ color: '#e0f0ff', fontWeight: 800, fontSize: '22px', margin: 0, letterSpacing: '-0.3px' }}>
                Cardiovascular Monitor
              </h1>
              <p style={{ color: 'rgba(180,210,255,0.5)', fontSize: '13px', margin: '2px 0 0' }}>
                Real-time cardiac health tracking
              </p>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{
                padding: '6px 14px', borderRadius: '20px', fontSize: '12px', fontWeight: 700,
                background: `${zoneColor}22`, border: `1px solid ${zoneColor}55`, color: zoneColor,
                letterSpacing: '0.05em', textTransform: 'uppercase' as const, transition: 'all 0.5s',
              }}>
                {zone} Zone
              </div>
              ...
              <button
                className="heart-log-btn"
                onClick={() => setShowLogModal(true)}
                style={{
                  background: 'linear-gradient(135deg,#ef4444,#dc2626)',
                  border: 'none', borderRadius: '12px', padding: '10px 20px',
                  color: '#fff', fontWeight: 700, fontSize: '13px', cursor: 'pointer',
                  boxShadow: '0 4px 16px rgba(239,68,68,0.35)', transition: 'all .2s ease',
          ...
          <div style={{ padding: isMobile ? '16px' : '24px 28px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? '1fr 1fr' : '1fr 1fr 320px', gap: '20px', minHeight: 'calc(100vh - 73px)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              ...
              <div style={{
                ...
                border: '1px solid rgba(239,68,68,0.25)', borderRadius: '20px', padding: '28px',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px' }}>
                      {/* key prop remounts the span → CSS bpmPop animation restarts on each BPM tick */}
                      <span
                        key={bpmKey}
                        style={{ fontSize: '64px', fontWeight: 900, color: '#fff', lineHeight: 1, display: 'inline-block', animation: 'bpmPop 0.35s ease' }}
                      >
                        {bpm}
                      </span>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '14px' }}>
                {[
                  ...
                  <div key={stat.label} style={{
                    background: 'rgba(8,20,50,0.75)', backdropFilter: 'blur(20px)',
                    border: `1px solid ${stat.color}22`, borderRadius: '16px', padding: '18px 20px',
                    animation: mounted ? `fadeUp 0.5s ease ${0.15 + i * 0.07}s both` : 'none',
                    cursor: 'default',
                  }} className="metric-card">
              ...
              <div style={{
                background: 'rgba(8,20,50,0.75)', backdropFilter: 'blur(20px)',
                border: '1px solid rgba(251,191,36,0.25)', borderRadius: '18px', padding: '18px 22px',
                ...
                <div style={{ display:'flex', gap:'5px', flexShrink:0 }}>
                  {Array.from({length:7}).map((_,i) => (
                    <div key={i} style={{ width:8, height:8, borderRadius:'50%', background: i < 7 ? '#fbbf24' : 'rgba(255,255,255,0.1)', boxShadow: i < 7 ? '0 0 6px rgba(251,191,36,0.6)' : 'none', transition:`all 0.3s ease ${i*0.05}s` }} />
                  ))}
                </div>
              </div>

              {/* Track My Mood Banner */}
              <div style={{
                background: 'linear-gradient(135deg, rgba(99,102,241,0.35) 0%, rgba(14,165,233,0.35) 100%)',
                backdropFilter: 'blur(20px)', border: '1px solid rgba(99,102,241,0.4)', borderRadius: '18px', padding: '22px 24px',
                ...
                <div>
                  ...
                  <p style={{ color: 'rgba(180,210,255,0.6)', fontSize: '12px', margin: 0, maxWidth: '260px' }}>
                    Take a moment to unwind with guided breathing exercises and mindfulness tips.
                  </p>
                </div>
                <button
                  className="track-btn"
                  ...
                  style={{
                    background: 'linear-gradient(135deg, #6366f1, #0ea5e9)', border: 'none', borderRadius: '12px', padding: '12px 22px',
                    color: '#fff', fontWeight: 700, fontSize: '13px', cursor: 'pointer',
                    whiteSpace: 'nowrap' as const, boxShadow: '0 6px 20px rgba(99,102,241,0.35)',
                    letterSpacing: '0.02em', flexShrink: 0, marginLeft: '16px',
                  }}
                >
                  Track My Mood
                </button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              ...
              {/* Heart & Lungs — 160px images */}
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '14px' }}>

                {/* Heart Card */}
                <div style={{
                  background: 'rgba(8,20,50,0.75)', backdropFilter: 'blur(20px)',
                  border: '1px solid rgba(239,68,68,0.25)', borderRadius: '20px', padding: '24px 20px',
                  ...
                  <img src={heartImg} alt="Heart" style={{
                    width: '160px', height: '160px', objectFit: 'contain',
                    animation: 'heartPulse 1.8s ease-in-out infinite',
                    filter: 'drop-shadow(0 0 28px rgba(239,68,68,0.55))',
                  }} />
                ...
                <div style={{
                  background: 'rgba(8,20,50,0.75)', backdropFilter: 'blur(20px)',
                  border: '1px solid rgba(96,165,250,0.25)', borderRadius: '20px', padding: '24px 20px',
                  display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: '14px',
                  ...
                  <img src={lungsImg} alt="Lungs" style={{
                    width: '160px', height: '160px', objectFit: 'contain',
                    animation: 'lungBreath 3.5s ease-in-out infinite',
              ...
              <div style={{
                ...
                animation: mounted ? 'fadeUp 0.5s ease 0.3s both' : 'none', flex: 1,
              }}>
                    ...
                    return (
                      <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: '4px' }}>
                        <div style={{
                          ...
                          background: isToday ? 'linear-gradient(180deg, #ef4444, #7f1d1d)' : 'rgba(100,180,255,0.18)',
                          border: isToday ? '1px solid rgba(239,68,68,0.5)' : '1px solid rgba(100,180,255,0.1)',
              ...
              <div style={{
                background: 'rgba(8,20,50,0.75)', backdropFilter: 'blur(20px)',
                border: '1px solid rgba(100,180,255,0.15)', borderRadius: '20px', padding: '20px',
                display: 'flex', justifyContent: 'space-around', alignItems: 'center',
                animation: mounted ? 'fadeUp 0.5s ease 0.35s both' : 'none',
              }}>
      ...
      {showLogModal && (
        ...
        >
          <div
            style={{ background:'#0d1a38', border:'1px solid rgba(239,68,68,0.3)', borderRadius:'22px', padding:'36px', width:'100%', maxWidth:'460px', boxShadow:'0 20px 60px rgba(0,0,0,0.6)', animation:'fadeUp .3s ease' }}
          ...
          >
            ...
            <form onSubmit={e => { e.preventDefault(); toast.success('Activity logged!'); setShowLogModal(false); setActivityData({ type:'Running', duration:'', intensity:'5', notes:'' }); }}>
              ...
              <div style={{ display:'flex', gap:'10px' }}>
                <button type="submit" style={{ flex:1, padding:'13px', background:'linear-gradient(135deg,#ef4444,#dc2626)', border:'none', borderRadius:'12px', color:'#fff', fontWeight:700, fontSize:'14px', cursor:'pointer', boxShadow:'0 4px 18px rgba(239,68,68,0.35)', transition:'all .2s' }}
